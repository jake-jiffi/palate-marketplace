...
/contact     0.191   MOVED              (baseline 2026-07-12)
             If that was intended, accept it and re-baseline with /palate-beta:drift --rebaseline.
...
Whether the move was an improvement is the paid half. The library percentile comes from `palate_taste_score` (which `grade-local.mjs` calls); the shareable number out of 100 comes from /palate-beta:grade. They are two different numbers and this paragraph exists to keep them apart.
```
