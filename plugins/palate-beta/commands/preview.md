...
This command never gates. It builds, serves, shoots and shows. `/palate-beta:publish` is what refuses.
...
error will not surface here: it surfaces at `/palate-beta:publish`, which does build.
...
- The next step: `/palate-beta:publish` when it looks right.
