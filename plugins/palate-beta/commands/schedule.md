...
Hand to `/palate-beta:publish`. A draft is invisible in production, so shipping it now is safe and it
...
scheduler has to run `/palate-beta:schedule --due` at that time. Offer the two real options, a
...
Hand to `/palate-beta:publish`, once, for all the posts released in this run. One deploy, not one per
post.
