...
say where the subject is. That is `/palate-beta:image`'s job, not adoption's.
...
the reason `/palate-beta:fact` will not do much here yet.
...
> `/palate-beta:check` before any change, `/palate-beta:drift` any time to see what has moved.
